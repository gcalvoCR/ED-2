...
void ArbolB::setOrden(int orden)
{
	this->orden = orden;
}
...
bool ArbolB::buscar(int a) {
	...
	Pagina* p;
	Pagina* actual; // actual es la pagina donde seguira buscando
	p = raiz;
...
/*  funcion que agrega un numero al arbol b, tiene como parametro el numero 'a'
	a agregar */
bool ArbolB::agregar(int a) {

	if (buscar(a))	return buscar(a); // busca el numero 'a'
...
string ArbolB::pintarArbol() {
	string rslt = "";
	Casilla2* p;
	p = principio;
	//p=final;
	if (!p) {
		rslt += "No se encuentra ningun numero en el arbol!";
	}
	while (p) {
		rslt += "valor: " + to_string(p->direccionArbol->valor) + "\n";
		...
		if (p->direccionArbol->hizq) {
			rslt += p->direccionArbol->hizq->pintar();
		}
		else rslt += "NO TIENE";

		rslt += "  HDer: ";
		if (p->direccionArbol->hder) {
			rslt += p->direccionArbol->hder->pintar();
		}
		else rslt += "NO TIENE";
		...
		if (p->direccionArbol->actual->obtenerPadre()) {
			rslt += "  Padre: ";
			rslt += p->direccionArbol->actual->obtenerPadre()->pintar();
		}
		else {
			rslt += "  Padre: No tiene padre es la raiz";
		}

		if (p->direccionArbol->actual->obtenerPadreCasilla()) {
			rslt += "  PadreCasilla: " + to_string(p->direccionArbol->actual->obtenerPadreCasilla()->valor);
		}
		else {
			rslt += "  PadreCasilla: No tiene padre es la raiz";
		}
...
string ArbolB::pintar() {
	...
	while (p) {
		if (p->siguiente) {
			rslt += "|" + to_string(p->direccionArbol->valor) + "|-";
		}
		else {
			rslt += "|" + to_string(p->direccionArbol->valor) + "|\n";
		}
